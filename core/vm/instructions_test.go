...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		opFn(&pc, evmInterpreter, &ScopeContext{
			Memory:   &Memory{},
			Stack:    stack,
			Contract: &Contract{},
			Metadata: &metadata,
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		stack.push(x)
		var metadata = NewMetadata()
		opAddmod(&pc, evmInterpreter, &ScopeContext{
			Memory:   &Memory{},
			Stack:    stack,
			Contract: &Contract{},
			Metadata: &metadata,
...
func getResult(args []*twoOperandParams, opFn executionFunc) []TwoOperandTestcase {
	...
	for i, param := range args {
		...
		stack.push(y)
		var metadata = NewMetadata()
		opFn(&pc, interpreter, &ScopeContext{
			Memory:   &Memory{},
			Stack:    stack,
			Contract: &Contract{},
			Metadata: &metadata,
...
func opBenchmark(bench *testing.B, op executionFunc, args ...string) {
	...
	for i := 0; i < bench.N; i++ {
		...
		var metadata = NewMetadata()
		op(&pc, evmInterpreter, &ScopeContext{
			Memory:   &Memory{},
			Stack:    stack,
			Contract: &Contract{},
			Metadata: &metadata,
...
func TestOpMstore(t *testing.T) {
	...
	var metadata = NewMetadata()
	opMstore(&pc, evmInterpreter, &ScopeContext{
		Memory:   mem,
		Stack:    stack,
		Contract: &Contract{},
		Metadata: &metadata,
...
func BenchmarkOpSHA3(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		var metadata = NewMetadata()
		opSha3(&pc, evmInterpreter, &ScopeContext{
			Memory:   mem,
			Stack:    stack,
			Contract: &Contract{},
			Metadata: &metadata,
